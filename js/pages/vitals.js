...
// the two scrollable lists were capped. Steps now comes from the Google
// Health API (server/google_health.py) via api.js's cache, averaged over
// whatever days in the 15-day window actually have a synced value.
// ---------------------------------------------------------------------------
...
export function renderBody(){
  const d=db(),now=new Date();const labels=[],sleepData=[],hrData=[];const stepVals=[];
  for(let i=14;i>=0;i--){
    const dt=addDays(now,-i);const ds=dateStr(dt);labels.push((dt.getMonth()+1)+'/'+dt.getDate());
    const sl=d.sleeps.find(s=>s.date===ds);const hr=d.hrs.find(h=>h.date===ds);
    sleepData.push(sl?+(sl.hours):null);hrData.push(hr?+(hr.resting):null);
    const v=getCachedVitals(ds);if(v&&+v.steps>=0&&v.steps!=null)stepVals.push(+v.steps);
  }
